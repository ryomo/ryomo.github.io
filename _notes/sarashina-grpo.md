...
Jupyter Labで`notebooks/jupyter`ディレクトリにある`inference.ipynb`を開いてください。
...
次に、`notebooks/jupyter`ディレクトリにある`finetuning.ipynb`を開いてください。再度「►►」こんな感じのボタンを押して、全てのセルを実行してください。
